...
class Omnisend_Helper {
	...
	public static function check_wp_wc_compatibility() {
		...
		$wc_plugin_readme_path = WC()->plugin_path() . '/readme.txt';
		$version_line          = '';

		if ( file_exists( $wc_plugin_readme_path ) ) {
			$fh = fopen( $wc_plugin_readme_path, 'r' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
			while ( true ) {
				$line = fgets( $fh );
			...
			$wc_supported_version = str_replace( 'Requires at least: ', '', $version_line );

			if ( floatval( get_bloginfo( 'version' ) ) < floatval( $wc_supported_version ) ) {
				return false;
			} else {
				return true;
			}
	...
	public static function omnisend_plugin_version() {
		$wc_path      = __DIR__ . '/../readme.txt';
		$version_line = '';
		$version      = '';
		if ( file_exists( $wc_path ) ) {
			$fh = fopen( $wc_path, 'r' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
			while ( true ) {
				$line = fgets( $fh );
			...
			fclose( $fh ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose
			$version = str_replace( 'Stable tag: ', '', $version_line );
