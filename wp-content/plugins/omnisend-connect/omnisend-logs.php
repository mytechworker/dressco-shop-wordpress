...
function omnisend_show_logs() {
	...
	if ( isset( $_GET['action'] ) && check_admin_referer( 'omnisend_logs' ) ) {
		if ( $_GET['action'] == 'log_options' ) {
			...
			wp_safe_redirect( admin_url( 'admin.php?page=' . OMNISEND_LOGS_PAGE ) );
			exit;
		}
	}
	// phpcs:enable
	...
	<div class="settings-page">
		...
		omnisend_display_title( 'Log file' );
			...
			if ( $logging_enabled ) {
				echo "<div class='logging_status logging_enabled'>Logs are  enabled. <a href='" . esc_url(
					add_query_arg(
						...
						admin_url( 'admin.php?page=' . OMNISEND_LOGS_PAGE . '&action=log_options' )
					)
				) . "'>Disable</a> </div>";
			} else {
		...
		echo "<p><a href='" . esc_url(
			add_query_arg(
				...
				admin_url( 'admin.php?page=' . OMNISEND_LOGS_PAGE . '&action=clean_log' )
			)
