...
define( 'OMNISEND_API_URL', 'https://api.omnisend.com/v3/' );
...
require_once 'view/settings/title.php';
...
function omnisend_woocommerce_menu() {
	...
	$menu_title = get_menu_title( 'Omnisend for Woo' );
	...
	add_submenu_page( $menu_slug, __( 'Logs' ), __( 'Logs' ), $capability, OMNISEND_LOGS_PAGE, 'omnisend_show_logs' );
	if ( get_option( 'omnisend_api_key', null ) !== null ) {
		add_submenu_page(
			$menu_slug,
			__( 'Sync' ),
			__( 'Sync' ),
...
function omnisend_admin_scripts_and_styles() {
	...
	if ( isset( $_GET['page'] ) ) {
		if ( in_array( $_GET['page'], array( OMNISEND_SETTINGS_PAGE, OMNISEND_LOGS_PAGE, OMNISEND_SYNC_PAGE ), true ) ) {
...
function omnisend_show_connection_notice() {
	if ( get_option( 'omnisend_api_key', null ) ) {
		return;
	}
	?>
	<div class="notice notice-warning is-dismissible omnisend-connection-notice-hidden">
...
function omnisend_show_api_access_notice() {
	if ( Omnisend_Manager::is_setup() && ! Omnisend_Helper::is_woocommerce_api_access_granted() ) {
		...
		?>
		<div class="notice notice-error is-dismissible omnisend-connection-notice-hidden">
			<p>
