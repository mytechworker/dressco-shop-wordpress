...
function omnisend_checkbox_custom_checkout_field( $checkout ) {
	...
	$connected = ! ! get_option( 'omnisend_api_key', null );
...
function omnisend_update_contact_status( $order_id ) {
	...
	if ( isset( $_POST['omnisend_newsletter_checkbox'] ) && sanitize_text_field( wp_unslash( $_POST['omnisend_newsletter_checkbox'] ) ) ) {
		...
		$link = OMNISEND_API_URL . 'contacts';
...
function omnisend_setup_omnisend_settings() {
	...
	Omnisend_Settings::add_contact_tag_status( map_value_to_status( Omnisend_Settings::get_contact_tag() ), Omnisend_Settings::SOURCE_DEFAULT );

	Omnisend_Settings::add_checkout_opt_in_text( '', Omnisend_Settings::SOURCE_DEFAULT );
	Omnisend_Settings::add_checkout_opt_in_status( map_value_to_status( Omnisend_Settings::get_checkout_opt_in_text() ), Omnisend_Settings::SOURCE_DEFAULT );
	Omnisend_Settings::add_checkout_opt_in_preselected_status( false );
}

function map_value_to_status( $value ) {
...
function detect_omnisend_plugin_updates() {
	...
	if ( is_admin() && ! empty( get_option( 'omnisend_api_key', null ) ) ) {
		...
		if ( $version_db != $omnisend_plugin_version ) {
